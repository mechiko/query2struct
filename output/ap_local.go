...
type ApLocalSlice []*ApLocal

func (r *ApLocal) Insert(db *sqlx.DB) (err error) {
	sql := `INSERT INTO ap_local (product_full_name,product_capacity,product_alc_volume,product_alc_code,product_code,product_unit_type,producer_type,producer_client_reg_id,producer_inn,producer_kpp,producer_full_name,producer_short_name,producer_country_code,producer_region_code,producer_description) 
	VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);`
...
func (r *ApLocal) Update(db *sqlx.DB) (err error) {
	if r.Id == 0 {
		return r.Insert(db)
	}
	sql := `UPDATE ap_local SET product_full_name=?,product_capacity=?,product_alc_volume=?,product_alc_code=?,product_code=?,product_unit_type=?,producer_type=?,producer_client_reg_id=?,producer_inn=?,producer_kpp=?,producer_full_name=?,producer_short_name=?,producer_country_code=?,producer_region_code=?,producer_description=?
	WHERE id=?;`
...
func (r *ApLocal) Delete(db *sqlx.DB) (err error) {
	if r.Id == 0 {
		return fmt.Errorf("id = 0")
	}
	sql := `DELETE ap_local WHERE id=?;`
	_, err = db.Exec(sql, r.Id)
	return err
}

func (r *ApLocal) Find(db *sqlx.DB) (err error) {
	if r.Id == 0 {
		return fmt.Errorf("r.Id = 0")
	}
	sql := `select * from ap_local WHERE id=?;`
	err = db.Get(r, sql, r.Id)
	return err
}

func (r *ApLocal) All(db *sqlx.DB) (out ApLocalSlice, err error) {
	out = make(ApLocalSlice, 0)
	sql := `select * from ap_local order by id;`
