...
type ApEgaisSlice []*ApEgais

func (r *ApEgais) Insert(db *sqlx.DB) (err error) {
	sql := `INSERT INTO ap_egais (id_requests,product_full_name,product_capacity,product_alc_volume,product_alc_code,product_code,product_unit_type,producer_type,producer_client_reg_id,producer_inn,producer_kpp,producer_full_name,producer_short_name,producer_country_code,producer_region_code,producer_description) 
	VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);`
...
func (r *ApEgais) Update(db *sqlx.DB) (err error) {
	if r.Id == 0 {
		return r.Insert(db)
	}
	sql := `UPDATE ap_egais SET id_requests=?,product_full_name=?,product_capacity=?,product_alc_volume=?,product_alc_code=?,product_code=?,product_unit_type=?,producer_type=?,producer_client_reg_id=?,producer_inn=?,producer_kpp=?,producer_full_name=?,producer_short_name=?,producer_country_code=?,producer_region_code=?,producer_description=?
	WHERE id=?;`
...
func (r *ApEgais) Delete(db *sqlx.DB) (err error) {
	if r.Id == 0 {
		return fmt.Errorf("id = 0")
	}
	sql := `DELETE ap_egais WHERE id=?;`
	_, err = db.Exec(sql, r.Id)
	return err
}

func (r *ApEgais) Find(db *sqlx.DB) (err error) {
	if r.Id == 0 {
		return fmt.Errorf("r.Id = 0")
	}
	sql := `select * from ap_egais WHERE id=?;`
	err = db.Get(r, sql, r.Id)
	return err
}

func (r *ApEgais) All(db *sqlx.DB) (out ApEgaisSlice, err error) {
	out = make(ApEgaisSlice, 0)
	sql := `select * from ap_egais order by id;`
